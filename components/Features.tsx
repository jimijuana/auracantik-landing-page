...
const Features: React.FC = () => {
      ...
      mockup: (
        <div className="relative h-full w-full bg-aura-offwhite p-6 flex flex-col justify-center overflow-hidden">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-3xl p-5 shadow-xl shadow-aura-mahogany/5 border border-aura-mahogany/5 translate-x-[-12%] animate-[bounce_4s_infinite]">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-full bg-aura-red/10 flex items-center justify-center text-aura-red font-serif font-bold text-sm">V</div>
                <div>
                  <div className="h-2 w-20 bg-aura-mahogany/20 rounded-full mb-1" />
                  <div className="h-1.5 w-12 bg-aura-mahogany/10 rounded-full" />
                </div>
              </div>
              <div className="h-9 bg-aura-mahogany rounded-2xl flex items-center justify-center text-[9px] font-black text-white uppercase tracking-widest">Confirm Call</div>
            </div>
            <div className="bg-white/40 backdrop-blur-md rounded-3xl p-5 border border-white/60 translate-x-[12%]">
      ...
      mockup: (
        <div className="relative h-full w-full bg-[#111111] overflow-hidden">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="w-40 h-40 bg-aura-red/10 rounded-full animate-ping" />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative">
                ...
                <div className="absolute top-[-46px] left-1/2 -translate-x-1/2 bg-white px-3 py-1.5 rounded-lg shadow-2xl whitespace-nowrap">
                  <span className="text-[8px] font-black text-aura-mahogany tracking-wider">PREMIUM ARTIST FOUND</span>
                </div>
              </div>
            </div>
          </div>
      ...
      mockup: (
        <div className="relative h-full w-full p-8 flex flex-col bg-white">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-14 h-14 rounded-2xl bg-aura-mahogany shadow-xl flex items-center justify-center">
    ...
    },
    {
      ...
      mockup: (
        <div className="relative h-full w-full p-6 flex flex-col justify-end gap-3 bg-white">
          <div className="absolute top-0 left-0 right-0 h-14 border-b border-aura-mahogany/5 flex items-center px-6">
          ...
          <div className="self-end max-w-[85%] bg-aura-mahogany p-4 rounded-[1.5rem] rounded-br-none text-[10px] text-white">
            Thank you. I'm available. Let's discuss the moodboard!
          </div>
  ...
  return (
    <section ref={sectionRef} className="py-28 md:py-36 px-6 bg-white border-t border-aura-mahogany/5">
      <div className="max-w-[1400px] mx-auto">
        <div className="text-center mb-24 md:mb-32">
          <span className="text-aura-red text-[9px] font-black uppercase tracking-[0.8em] mb-8 inline-block">Core Capabilities</span>
          <h2 className="text-[clamp(2.5rem,5vw,6rem)] font-serif font-bold text-aura-mahogany leading-[0.9] tracking-tighter mb-8">
            Engineered for <br /> <span className="italic">Excellence</span>
          </h2>
          <p className="max-w-xl mx-auto text-aura-mahogany/40 text-sm font-light leading-relaxed">
            Every tool you need to thrive in Malaysia's most competitive visual industries, built into one seamless platform.
          </p>
        </div>

        <div ref={gridRef} className="grid md:grid-cols-2 gap-8 md:gap-12">
          {featureData.map((feature, idx) => (
            <div key={idx} className="group relative bg-aura-offwhite rounded-[3rem] p-10 md:p-14 border border-transparent transition-all duration-700 hover:bg-white hover:border-aura-mahogany/5 hover:shadow-[0_60px_120px_-30px_rgba(32,19,18,0.08)] hover:-translate-y-1">
              <div className="relative z-10 flex flex-col lg:flex-row gap-10 items-center">
                <div className="flex-1 order-2 lg:order-1">
                  <div className="flex items-center gap-4 mb-8">
                    {/* Number badge */}
                    <span className="text-[10px] font-black text-aura-mahogany/15 font-serif">{feature.num}</span>
                    <div className="w-11 h-11 bg-white rounded-2xl flex items-center justify-center shadow-md text-aura-red group-hover:bg-aura-red group-hover:text-white transition-all duration-500">
                      {feature.icon}
                    </div>
                    <span className="text-[9px] font-black tracking-[0.45em] text-aura-red/50">{feature.tag}</span>
                  </div>
                  <h3 className="text-3xl md:text-4xl font-serif font-bold text-aura-mahogany mb-4 leading-[1.1]">{feature.title}</h3>
                  <p className="text-aura-mahogany/50 text-base font-light leading-relaxed mb-8">{feature.desc}</p>

                  <a href="#get-app" onClick={(e) => { e.preventDefault(); const el = document.getElementById('get-app'); if (el) window.scrollTo({ top: el.offsetTop - 100, behavior: 'smooth' }); }} className="inline-flex items-center gap-3 text-[9px] font-black uppercase tracking-[0.4em] text-aura-mahogany/40 hover:text-aura-red transition-all group/btn">
                    Learn More
                    <div className="w-6 h-[1.5px] bg-aura-mahogany/20 group-hover/btn:w-12 group-hover/btn:bg-aura-red transition-all duration-500" />
                  </a>
                </div>

                <div className="flex-1 order-1 lg:order-2 aspect-square rounded-[2.5rem] bg-white border border-aura-mahogany/5 shadow-inner overflow-hidden flex items-center justify-center transform group-hover:scale-[1.02] transition-transform duration-700">
                  {feature.mockup}
                </div>
              </div>
            </div>
